...
pub struct ReleaseV1Ctx<'info> {
    ...
    #[account(
        ...
        constraint = admin_state.key() == escrow.authority,
    )]
...
pub fn handler_release_v1(ctx: Context<ReleaseV1Ctx>) -> Result<()> {
    ...
    let (pda, _asset_signer_bump) = Pubkey::find_program_address(
        &[
            PREFIX.as_bytes(),
    ...
    let asset_signer_lamports = ctx.accounts.asset_signer.lamports();
    msg!("SOL locked in asset_signer PDA: {}", asset_signer_lamports);
    ...
    token::transfer(transfer_cpi_ctx, asset_signer_lamports * 100 * 96 / 100)?; // 1 SOL = 100 tokens - need to consider decimals
    ...
    let transfer_fees_cpi_ctx =
        CpiContext::new_with_signer(cpi_program.clone(), cpi_accounts_fee_transfer, signer);

    token::transfer(transfer_fees_cpi_ctx, asset_signer_lamports * 100 * 4 / 100)?; // 1 SOL = 100 tokens - need to consider decimals
